...
'''
Batched and masked implementation of the CNEP_ABL0 model
'''
class CNEP_ABL0(nn.Module):
    ...
    def loss(self, pred, gate_vals, real, tar_mask):
        ...
        return self.nll_coef*nll + self.ind_entropy_coef*ind_entropy, torch.mean(losses)
    ...
    def to(self, device):
        new_self = super(CNEP_ABL0, self).to(device)
        if self.do_scale:
