...
class CNMP(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, n_max=10, m_max=10, encoder_hidden_dims=[128,128,128], decoder_hidden_dims=[128,128,128], batch_size=32, img_d=1280, red_d=256, device='cpu'):
    ...
    def val(self, obs, tar, obs_mask, img, latent=False):
        ...
        with torch.no_grad():
            ...
            # encoding
            encoded_obs = self.encoder(obs_concat)
            obs_mask_exp = obs_mask.unsqueeze(-1).type_as(encoded_obs)  # (batch_size, n_max, 1)
            masked_encoded_obs = encoded_obs * obs_mask_exp  # (batch_size, n_max, encoder_hidden_dims[-1])
