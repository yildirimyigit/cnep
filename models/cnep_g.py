...
class CNEP(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, n_max=10, m_max=10, encoder_hidden_dims=[256,256,256],
                 num_decoders=4, decoder_hidden_dims=[128,128], batch_size=32, img_d=1280, red_d=256, nll_coef=16.81, batch_entropy_coef=10.672, ind_entropy_coef=7.553, scale_coefs=False, device='cuda') -> None:
        ...
        self.dim_reducer = nn.Linear(img_d, red_d)

        self.to(self.device)

    def forward(self, obs, tar, obs_mask, img, latent=False):
        # obs: (batch_size, n_max, input_dim+output_dim)
        # tar: (batch_size, m_max, input_dim)
        # obs_mask: (batch_size, n_max)
        # img: (batch_size, img_d)

        img_red = self.dim_reducer(img) / img.shape[1]  # (batch_size, red_d)
        obs_concat = torch.cat([obs, img_red.unsqueeze(1).expand(-1, self.n_max, -1)], dim=-1)  # (batch_size, n_max, input_dim+output_dim+red_d)

        # encoding
        encoded_obs = self.encoder(obs_concat)  # (batch_size,  encoder_hidden_dims[-1])
        ...
        tar_concat = torch.cat([tar, img_red.unsqueeze(1).expand(-1, self.m_max, -1)], dim=-1)  # (batch_size, m_max, input_dim+red_d)

        # decoding
        r_repeated = r.repeat(1, self.m_max, 1)
        rep_tar = torch.cat([r_repeated, tar_concat], dim=-1)
    ...
    def val(self, obs, tar, obs_mask, img, latent=False):
        # obs: (batch_size, n_max, input_dim+output_dim)
        # tar: (batch_size, t_steps, input_dim)
        # obs_mask: (batch_size, m_max)
        # img: (batch_size, img_d)

        with torch.no_grad():
            img_red = self.dim_reducer(img) / img.shape[1]
            obs_concat = torch.cat([obs, img_red.unsqueeze(1).expand(-1, self.n_max, -1)], dim=-1)

            # encoding
            encoded_obs = self.encoder(obs_concat)  # (batch_size, n_max, encoder_hidden_dims[-1])
            obs_mask_exp = obs_mask.unsqueeze(-1).type_as(encoded_obs)  # (batch_size, n_max, 1)
            masked_encoded_obs = encoded_obs * obs_mask_exp  # (batch_size, n_max, encoder_hidden_dims[-1])
            ...
            tar_concat = torch.cat([tar, img_red.unsqueeze(1).expand(-1, tar.shape[1], -1)], dim=-1)  # (batch_size, t_steps, input_dim+red_d)
    ...
    def loss(self, pred, gate_vals, real, tar_mask):
        # pred: (num_decoders, batch_size, m_max, 2*output_dim)
