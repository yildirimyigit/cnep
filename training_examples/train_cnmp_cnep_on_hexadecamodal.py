...
def generate_trajectories(num_demos=100, num_points=200):
    ...
    # Define 16 modes for the trajectories
    modes = [
        ...
        lambda x: 0.5 * torch.sin(2 * np.pi * x) + 2 * torch.cos(2 * np.pi * x),
        lambda x: 0.5 * torch.cos(2 * np.pi * x) + 2 * torch.sin(2 * np.pi * x),
        lambda x: torch.tan(np.pi * x),
        lambda x: -torch.tan(np.pi * x),
        lambda x: torch.sin(np.pi * x) - torch.cos(np.pi * x),
        lambda x: torch.cos(np.pi * x) - torch.sin(np.pi * x),
        lambda x: 1.5 * torch.sin(3 * np.pi * x) + torch.cos(3 * np.pi * x),
        lambda x: 1.5 * torch.cos(3 * np.pi * x) + torch.sin(3 * np.pi * x)
    ]
...
x, y = generate_trajectories(num_demos=1024)
vx, vy = generate_trajectories(num_demos=256)
...
root_folder = f'../outputs/sine/hexadecamodal/cnmp_cnep/{str(timestamp)}/'
