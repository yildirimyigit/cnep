...
sweep_config = {
    ...
    'parameters': 
    {
        ...
        'batch_entropy_coef': {'max': 20.0, 'min': -20.0},
        'ind_entropy_coef': {'max': 20.0, 'min': -20.0}
     }
}

# sweep_id = wandb.sweep(
#  sweep=sweep_config,
#  project='cos_sin_large-loss-components-sweep'
#  )
...
def train():
    ...
    batch_entropy_coef = wandb.config.batch_entropy_coef
    ind_entropy_coef = wandb.config.ind_entropy_coef

    model_wta = WTA_CNP(1, 1, n_max_obs, n_max_tar, [128, 128, 128], num_decoders=8, decoder_hidden_dims=[128, 128, 128], batch_size=batch_size,
                        nll_coef=nll_coef, batch_entropy_coef=batch_entropy_coef, ind_entropy_coef=ind_entropy_coef).to(device_wta)
    optimizer_wta = torch.optim.Adam(lr=1e-4, params=model_wta.parameters())

    timestamp = int(time.time())
    root_folder = f'outputs/cos_sin_large/{str(timestamp)}/'
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            print("Epoch: {}, WTA-Loss: {}".format(epoch, avg_loss_wta/100))
            avg_loss_wta = 0
...
def draw_val_plot(root_folder, epoch, model_wta):
...
# %%
wandb.agent(sweep_id, function=train, count=100, project='cos_sin_large-loss-components-sweep')
