...
img_in = torch.zeros((batch_size, dims), dtype=torch.float32, device=device)
...
val_img_in = torch.zeros((batch_size, dims), dtype=torch.float32, device=device)
...
# %%
cnep_ = CNEP(dx+dg, dy, n_max, n_max, [256, 256], num_decoders=2, decoder_hidden_dims=[128, 128], batch_size=batch_size, scale_coefs=True, device=device)
optimizer_cnep = torch.optim.Adam(lr=3e-4, params=cnep_.parameters())

cnmp_ = CNMP(dx+dg, dy, n_max, m_max, [256, 256], decoder_hidden_dims=[256, 256], batch_size=batch_size, device=device)
...
root_folder = f'../outputs/baxter/cnmp_cnep/wo_mobile_net/{str(timestamp)}/'
...
for epoch in range(epochs):
    ...
    for i in range(epoch_iter):
        ...
        pred, gate = cnep(obs, tar_x, obs_mask, img_in)
    ...
    if epoch % val_per_epoch == 0:
        with torch.no_grad():
            ...
            for j in range(v_epoch_iter):
                ...
                p, g = cnep.val(val_obs, val_tar_x, val_obs_mask, val_img_in)
